...
public class AdapterAutoTextDataGuruNama extends ArrayAdapter<DataModel> {
    private List<DataModel> dmListFull;

    public AdapterAutoTextDataGuruNama(@NonNull Context context, @NonNull List<DataModel> countryList) {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        TextView textViewTingkat = convertView.findViewById(R.id.tvNIPGuru);

        DataModel dm = getItem(position);

        if (dm != null) {
            textViewName.setText(dm.getNama());
            textViewTingkat.setText(dm.getNip());
        }
